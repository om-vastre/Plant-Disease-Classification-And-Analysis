...
#app.config['RESPONSE_HEADERS'] ={'Content-Type': 'multipart/form-data',}
...
def prediction():
    ...
    predictions = random.choice(labels)
    predicted_class = labels.index(predictions)


    try:
        desc = "Recommended Treatment :\n"+pesticides[labels[predicted_class]]
    except:
        desc=""

    return render_template("result.html", cla=labels[predicted_class], title=titles[labels[predicted_class]], desc=desc)
